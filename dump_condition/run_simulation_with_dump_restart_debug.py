...
def parse_bool(value):
    """Safely interpret YAML-like boolean values."""
    ...
    if isinstance(value, str):
        ...
        if normalized in {"false", "no", "off", "0", ""}:
            return False
    raise ValueError(
        f"simulation.dump must be true or false, not {value!r}."
    )
...
def run_simulation(config):
    ...
    dump_enabled = parse_bool(config["simulation"].get("dump", False))

    print(f"Saving SimulationArchive to: {output_file}")
    print(f"Dump enabled: {dump_enabled}")

    if dump_enabled:
        # Create the readable dump immediately so its location is unambiguous.
        dump_file.touch(exist_ok=True)
        print(f"Orbit dump path: {dump_file.resolve()}")
        print(f"Checkpoint path: {checkpoint_file.resolve()}")
    ...
    if not resumed:
        ...
        if dump_enabled:
            dump_file.touch()
            print(f"Created new orbit dump: {dump_file.resolve()}")
